...
Partial Class ListRecords
  ...
  Const myVerb As String = "ListRecords"

  Const mySessionVarObjSR As String = "objSR"
  Const mySessionVarPathInfo As String = "pathInfo"

  Const myXSLTStylesheet As String = "SR-ListRecords.xsl"
  Const myAppVarXSLTDoc As String = "SR-ListRecords-XSLT-Doc"

  ''' <summary>
  ''' Respond to the ListRecords verb
  ''' </summary>
  ''' <remarks></remarks>
  Sub ListRecords()
    ...
    If HttpContext.Current.IsDebuggingEnabled Then
      ...
      Response.Write("<!-- $Workfile: ListRecords.aspx.vb $ $Revision: 1 $ $Date: 2/09/07 10:23a $ -->" & vbCrLf)
    End If
    ...
    For Each strItem In attrArray
      If (Not IsNothing(Request.Params.GetValues(strItem)) AndAlso Request.Params.GetValues(strItem).Length = 1) Then
        strAttr = strAttr & " " & strItem & "=""" & Server.HtmlEncode(Request(strItem)) & """"
      End If
    Next
  ...
  ''' <summary>
  ''' Check that the arguments are appopriate for a ListRecords request
  ''' </summary>
  ...
  Function CheckArguments(ByRef strErr As String) As Boolean
    ...
    ' until (optional)
    If (Not IsNothing(Request.Params.GetValues("until"))) Then
    ...
    If (Not IsNothing(Request.Params.GetValues("metadataPrefix"))) Then
