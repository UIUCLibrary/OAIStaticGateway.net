...
<xsl:stylesheet version="1.0"
	...
	exclude-result-prefixes="oai dc oai_dc SR ms">
  ...
  <!-- Transform the static repository into the response -->
    ...
    <xsl:if test="$debug='true'">
      <xsl:comment>$Workfile: SR-ListIdentifiers.xsl $ $Revision: 1 $ $Date: 2/08/08 11:53a $</xsl:comment>
    </xsl:if>
			...
			<xsl:otherwise>
				<xsl:element name="ListIdentifiers">
					<xsl:for-each select="$recordSet">
						<xsl:for-each select="*">
							<xsl:if test="local-name() = 'header' and namespace-uri()='http://www.openarchives.org/OAI/2.0/'">
